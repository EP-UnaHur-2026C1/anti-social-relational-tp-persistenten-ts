...
const postSchema = joi.object({
    descripcion: joi.string().max(1000).required().message({
    ...
    fechaPublicacion: joi.date().iso().required().message({
    ...
    userId: joi.number().integer().required().messages({  //Relacion de 1:N con user
        'number.base': 'El ID de usuario debe ser un número',
        'any.required': 'El ID de usuario es obligatorio para publicar',
    }),

    tags: joi.array().items()(joi.number().integer()).messages({ //Relacion de N:N con tags: es un array de id´s
    ...
    imagenes: joi.array()
        ...
        .min(1) //Requiere al menos una imagen
        .required()
        .messages({
